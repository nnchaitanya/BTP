...
public class BackGroundTasks implements Runnable{
	...
	public ArrayList<byte[]> getTimeStampList(String userName , ArrayList<String> fileNames){
		int nFiles = fileNames.size();
		ArrayList<byte[]> ret = new ArrayList<byte[]>();
		for (int i=0; i<nFiles; i++){
			String fileName = fileNames.get(i);
			int indexOfUnder = fileName.lastIndexOf('_');
			String rowKey = userName + ","  + fileNames.get(i).substring(0, indexOfUnder);
...
//		TODO change this to greater than 
		if (smallFilesSize>ObjectProtocol.ARCHIVE_SIZE_THRESHOLD){
			...
			ArrayList<byte[]> timeStampList = getTimeStampList(userName , files);
			...
			for (int i=0; i<files.size(); i++){	
				...
				try{
					...
					String rowKey = userName + "," + entryName;
